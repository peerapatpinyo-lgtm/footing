...
# =========================================================================
st.set_page_config(page_title="Enterprise Footing Suite V7.2", page_icon="📐", layout="wide")
...
st.title("📐 Enterprise Footing Suite (V7.2 - Dimension Validation)")
st.markdown("### ระบบวิเคราะห์ฐานราก: เพิ่มโมดูลกำหนดมิติกว้าง-ยาว และตรวจสอบขนาดขั้นต่ำตามมาตรฐาน วสท.")
st.markdown("---")
...
with st.sidebar:
    ...
    # คำนวณระยะห่างและระยะขอบมาตรฐานล่วงหน้าเพื่อใช้กำหนดขั้นต่ำ
    S_dist = 3.0 * pile_size
    E_dist = max(pile_size, 0.35)

    # คำนวณพิกัดเข็มอุดมคติเพื่อหาขอบเขตต่ำสุดทางเรขาคณิต
    if footing_shape_type == "ฐานรากสี่เหลี่ยม (Rectangular Footing)":
        ...
        L_min_geometry = (S_dist * math.sqrt(3)/2) + 2 * E_dist
    ...
    if dim_mode == "กำหนดขนาดเอง (Manual Override)" and footing_shape_type == "ฐานรากสี่เหลี่ยม (Rectangular Footing)":
        st.caption(f"⚠️ เกณฑ์มิติขั้นต่ำตามพิกัดเสาเข็ม: B ≥ {B_min_geometry:.2f} ม. | L ≥ {L_min_geometry:.2f} ม.")
        B_input = st.number_input("ระบุความกว้างแกน X (B, เมตร)", value=float(round(B_min_geometry, 2)), min_value=0.4, step=0.05)
        L_input = st.number_input("ระบุความยาวแกน Y (L, เมตร)", value=float(round(L_min_geometry, 2)), min_value=0.4, step=0.05)
        
        # ตรวจสอบและบังคับไม่ให้ต่ำกว่าเกณฑ์ความปลอดภัยเรขาคณิตป้องกันการคำนวณหลุดขอบ
...
# =========================================================================
# AS-BUILT FIELD SURVEY DATA EDITOR (TABLE 1)
# =========================================================================
st.markdown("### 📍 1. ข้อมูลสำรวจเสาเข็มหนีศูนย์หน้างานจริง (As-Built Field Survey Analysis)")
st.info("💡 **การเชื่อมโยงข้อมูล:** ตารางนี้จะส่งผลต่อรูปแปลนวิศวกรรมด้านล่างโดยตรง โดยระบบจะลากเวกเตอร์สีแดงแสดงการเยื้องศูนย์จริงเปรียบเทียบกับแบบ")
...
if net_min_edge_dist < 0.10:
    st.error(f"🚨 **[As-Built Edge Distance Alert]** เสาเข็มหนีศูนย์จนเหลือระยะห่างผิวถึงขอบฐานรากเพียง {net_min_edge_dist*100:.1f} ซม. (น้อยกว่าเกณฑ์ปลอดภัยจำเพาะ 10 ซม. เสี่ยงคอนกรีตแตก!)")
...
# =========================================================================
# ENGINEERING CALCULATIONS ROUTINES (SHEAR & STRESS)
# =========================================================================
...
with col_adv1:
    ...
    st.write(f"* ขนาดวิเคราะห์จริงหน้างาน: **{B_ft:.2f} x {L_ft:.2f} เมตร**")
    if dim_mode == "คำนวณอัตโนมัติ (Auto-Size)":
        st.success(f"✅ ขนาดฐานรากถูกคำนวณให้เป็นค่าขั้นต่ำที่ปลอดภัยที่สุดโดยอัตโนมัติ")
    else:
        st.success(f"✅ ขนาดใช้งานผ่านเกณฑ์มิติขั้นต่ำเชิงเรขาคณิต ($B \ge {B_min_geometry:.2f}, L \ge {L_min_geometry:.2f}$)")

with col_adv2:
    if not step_safe:
        st.error("🚨 **สถานะแรงเฉือน:** ไม่ผ่านเกณฑ์จำกัดกำลังเนื่องจากหน้าตัดแคบหรือบางเกินไป")
    else:
        st.success("✅ **สถานะแรงเฉือน:** ผ่านเกณฑ์ความปลอดภัยรอบขอบตอม่อทั้งหมด")

# =========================================================================
# 2D ENGINEERING BLUEPRINT WITH DIMENSION LINES (UPDATED)
# =========================================================================
if not is_structure_crashed:
    ...
    # กำหนดขอบเขตขยายรูปเพื่อให้เห็นเส้นบอกขนาดชัดเจน
    ...
    # วาดและลากเส้นเวกเตอร์หนีศูนย์จากตาราง 1
    ...
    # --- 📐 เพิ่มเส้นบอกขนาดกว้าง-ยาว (Dimension Lines) บนแปลน ---
    if footing_shape_type == "ฐานรากสี่เหลี่ยม (Rectangular Footing)":
        # เส้นบอกขนาดความกว้างแกน X (ด้านล่างรูป)
        ...
        # เส้นบอกขนาดความยาวแกน Y (ด้านซ้ายรูป)
        ...
        # สำหรับฐานสามเหลี่ยมบอกขนาด Max Width กับ Max Height แทน
        dim_y = min(y_coords) - 0.25
        ax_plan.annotate('', xy=(min(x_coords), dim_y), xytext=(max(x_coords), dim_y), arrowprops=dict(arrowstyle='<->', color='#2c3e50', lw=1.5))
        ax_plan.text(0, dim_y - 0.12, f"Max Width = {B_ft:.2f} m", ha='center', va='center', color='#2c3e50', fontweight='bold', fontsize=10)
    ...
    ax_plan.set_title(f"แปลนแสดงมิติและขนาด {B_ft:.2f} x {L_ft:.2f} ม. (Top View)", fontsize=11, fontweight='bold')
...
with tab1:
    ...
    with col_m1: st.metric("ความกว้างฐานราก (B)", f"{B_ft:.2f} เมตร")
    with col_m2: st.metric("ความยาวฐานราก (L)", f"{L_ft:.2f} เมตร")
    with col_m3: st.metric("ความหนารวมฐานราก (t)", f"{t_actual*100:.1f} ซม.")
    with col_m4: st.metric("เนื้อที่ผิวคอนกรีตฐานราก", f"{footing_area:.3f} ตร.ม.")
